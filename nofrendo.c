...
static int install_timer(float hertz)
{
...
static int internal_insert(const char *filename, system_t type)
{
   ...
   case system_nes:
      ...
      /* Update refresh rate after loading ROM */
...
{
   vidinfo_t video;
   float refresh_rate = 59.94f; // NTSC by default 
    ...
    * We read the ROM header before initializing the display.
    * This allows us to configure the correct pixel clock ONLY ONCE.
    */
   if (system_nes == type || system_autodetect == type) {
      if (rom_get_region(filename) == 1) {
         refresh_rate = 50.00f; // PAL
      }
   }

   if (osd_init(refresh_rate))
      return -1;
