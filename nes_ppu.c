...
/* External HAL functions for direct NES controller access */
extern void osd_nes_latch(uint8 value);
extern uint8 osd_nes_clock_read(uint32 address);
...
{
   return &ppu.nametab[(nt & 1) << 10];
}

uint8 ppu_get_bg_table(void)
{
   return (ppu.ctrl0 & PPU_CTRL0F_BGADDR);
}
...
static void ppu_setstrike(int x_loc)
{
   if (false == ppu.strikeflag)
   {
      ...
      /* Battletoads NTSC/PAL timing:
      ** In NTSC: 3 pixels per cpu cycle (exact).
      ** In PAL:  3.2 pixels per cpu cycle (approx 106 cycles per line).
      */
      if (nes_scanlines_per_frame == 312) {
         ppu.strike_cycle = ppu.scanline_base_cycles + (uint32)((float)x_loc / 3.2f);
      } else {
         ppu.strike_cycle = ppu.scanline_base_cycles + (x_loc / 3);
      }
   }
}
...
uint8 ppu_readhigh(uint32 address)
{
   ...
   {
   case PPU_JOY0:
      value = 0x40 | osd_nes_clock_read(address);
      break;

   case PPU_JOY1:
      /* Combine with software input (Zapper, etc.) */
      value = input_get(INP_ZAPPER | INP_JOYPAD1);
      value |= osd_nes_clock_read(address);
      break;
...
uint8 ppu_read(uint32 address)
{
   ...
   case PPU_STAT:
      ...
      else if (ppu.strike_cycle != (uint32)-1)
      {
         /* Battletoads Hack: if we are at or past the predicted hit cycle, set the flag */
         if (nes6502_getcycles(false) >= ppu.strike_cycle) {
            ppu.strikeflag = true;
            value |= PPU_STATF_STRIKE;
         }
...
void ppu_write(uint32 address, uint8 value)
{
   ...
   case PPU_VDATA:
	 ...
	 /* Always allow writing for hardware checks */
         PPU_MEM(addr) = value;
      }
      else
      {
...
/* Prediction routine to set strike_cycle at the start of scanline */
void ppu_spr0_hit_predict(int scanline, uint32 base_cycles)
{
   if (false == ppu.obj_on || ppu.strikeflag)
      return;

   obj_t *sprite_ptr = (obj_t *)ppu.oam;
   int sprite_y = (int)sprite_ptr->y_loc + 1;
   int sprite_height = (int)ppu.obj_height;

   /* Is Sprite 0 on this line? */
   if ((sprite_y <= scanline) && (sprite_y > (scanline - sprite_height)) && (sprite_y != 0) && (sprite_y < 240))
   {
      int y_offset = scanline - sprite_y;
      uint8 tile_index = sprite_ptr->tile;
      uint8 attrib = sprite_ptr->atr;
      uint32 vram_adr;
      ...
      /* Handle vertical line within the tile */
      if (y_offset > 7) vram_adr += 8; /* bottom half of 8x16 */
      vram_adr += (y_offset & 7);

      /* Find first non-transparent pixel in this row of the sprite */
      uint8 pat1 = PPU_MEM(vram_adr);
      uint8 pat2 = PPU_MEM(vram_adr + 8);
      int first_pixel = -1;

      if (attrib & OAMF_HFLIP) {
         /* Horizontally flipped: check bits from 0 to 7 */
         for (int i = 0; i < 8; i++) {
            if ((pat1 & (1 << i)) || (pat2 & (1 << i))) {
               first_pixel = 7 - i;
               break;
            }
         }
      } else {
         /* Normal: check bits from 7 down to 0 */
         for (int i = 7; i >= 0; i--) {
            if ((pat1 & (1 << i)) || (pat2 & (1 << i))) {
               first_pixel = 7 - i;
               break;
      ...
      if (first_pixel != -1) {
         ppu.scanline_base_cycles = base_cycles;
         /* Use the exact pixel hit location */
         ppu_setstrike(sprite_ptr->x_loc + first_pixel);
         /* This is a prediction, the real render will finalize it */
   ...
   for (sprite_num = 0; sprite_num < 64; sprite_num++, sprite_ptr++)
   {
      ...
      */
      ...
      if (++spritecount == PPU_MAXSPRITE)
      {
         ...
         /* break; */ /* DISABLED: Render more than 8 sprites per line */
      }
   }
...
static void ppu_renderscanline(bitmap_t *bmp, int scanline, bool draw_flag)
{
   ...
   /* Start of scanline - Synchronize Loopy registers according to PPU status */
   if (ppu.bg_on || ppu.obj_on)
   {
      if (0 == scanline)
      {
	 /* Start of Frame: Transfer complete (v = t) */
         /* Synchronizes Coarse X/Y, Nametable, and Fine Y */     
         ppu.vaddr = ppu.vaddr_latch;
      }
      else
      {
	 /* Start of Line: Horizontal Transfer (v: ....H.. ...XXXXX = t: ....H.. ...XXXXX) */
         /* Copies only the Coarse X and the horizontal Nametable bit */     
         ppu.vaddr &= ~0x041F;
         ppu.vaddr |= (ppu.vaddr_latch & 0x041F);
      }
   }

   /* Restore background rendering if the PPU is active.
      This is vital for mappers like MMC3 (Mario 3) to count lines correctly. */
   if (draw_flag || ppu.bg_on)
      ppu_renderbg(buf);

   /* For sprites, render if necessary for drawing or if possible a 0-hit sprite */
   if (true == ppu.drawsprites && (true == draw_flag || ppu.obj_on))
...
void ppu_scanline(bitmap_t *bmp, int scanline, bool draw_flag, uint32 base_cycles)
{
   if (scanline < NES_SCREEN_HEIGHT)
   {
      /* Save base cycles for mid-scanline timing (e.g. Sprite 0 hit) */
      ppu.scanline_base_cycles = base_cycles;
   ...
   else if ((nes_scanlines_per_frame - 1) == scanline)
   {
      ppu.stat &= ~(PPU_STATF_VBLANK | PPU_STATF_STRIKE);
      ppu.strikeflag = false;
