...
/* Constants for wave centering */
#define TRIANGLE_CENTER 4096
#define DMC_CENTER      16384

/* Standardized output macros to avoid clipping and improve clarity */
#define APU_RECTANGLE_OUTPUT(channel) (apu.rectangle[channel].output_vol >> 2)
#define APU_TRIANGLE_OUTPUT ((apu.triangle.output_vol - TRIANGLE_CENTER) >> 2)
#define APU_NOISE_OUTPUT (apu.noise.output_vol >> 2)
#define APU_DMC_OUTPUT ((apu.dmc.output_vol - DMC_CENTER) >> 2)

/* active APU */
static apu_t apu;

/* Circular buffer for CPU synchronization */
#define APU_INTERNAL_BUF_SIZE 4096
static int16 apu_internal_buf[APU_INTERNAL_BUF_SIZE];
static volatile int apu_write_ptr = 0;
static volatile int apu_read_ptr = 0;

static void apu_append_sample(int16 sample)
{
   apu_internal_buf[apu_write_ptr] = sample;
   apu_write_ptr = (apu_write_ptr + 1) % APU_INTERNAL_BUF_SIZE;
}
...
{
   /* NO APU_VOLUME_DECAY for Triangle to preserve DC offset and avoid aliasing noise */
...
static void fill_dmc_buffer(void)
{
   if (apu.dmc.buffer_empty && apu.dmc.dma_length)
   {
      apu.dmc.sample_buffer = nes6502_getbyte(apu.dmc.address);
...
static int32 apu_dmc(void)
{
   ...
   if (apu.dmc.dma_length || !apu.dmc.silence || !apu.dmc.buffer_empty)
   {
      ...
      if (apu.dmc.accum < 0)
      {
         while (apu.dmc.accum < 0)
         {
            ...
            if (!apu.dmc.silence)
            {
               if (apu.dmc.shift_reg & 1)
               {
                  if (apu.dmc.regs[1] < 126) apu.dmc.regs[1] += 2;
               }
               else
               {
                  if (apu.dmc.regs[1] > 1) apu.dmc.regs[1] -= 2;
               }
               apu.dmc.output_vol = (apu.dmc.regs[1] << 8);
            }

            total += apu.dmc.output_vol;
            num_times++;

            apu.dmc.shift_reg >>= 1;

            if (--apu.dmc.bits_remain == 0)
            {
               apu.dmc.bits_remain = 8;
               if (apu.dmc.buffer_empty)
               {
                  apu.dmc.silence = true;
                  if (0 == apu.dmc.dma_length && apu.dmc.irq_gen)
                  {
         ...
         return (( (total / num_times) - DMC_CENTER) >> 2);
      }
   }
...
void apu_write(uint32 address, uint8 value)
{
   ...
   case APU_WRE1: /* 7-bit DAC */
      ...
      apu.dmc.output_vol = (value << 8); /* Absolute write, syncs with DC level */
      break;
   ...
   case APU_WRE3:
      ...
      apu.dmc.cached_dmalength = (value << 4) + 1; /* In bytes */
      break;
...
uint8 apu_read(uint32 address)
{
   ...
   case APU_SMASK:
      ...
      /* Actual DMC status: return 1 if there are bytes remaining */
...
void apu_process(void *buffer, int num_samples)
{
   ...
   static int16 last_valid_sample = 0;
   static bool pre_rolling = true;
   ...
   if (NULL == buffer)
   {
      /* Generate and store in incremental internal buffer */	   
      while (num_samples--)
      {
         ...
         /* do clipping */
         CLIP_OUTPUT16(accum);
         apu_append_sample((int16)accum);
      }
   }
   else
   {
      /* Copy from internal buffer to OSD with Soft Underflow protection */	   
      while (num_samples--)
      {
	 /* Calculate how many samples are available in the circular buffer */     
         int distance = (apu_write_ptr - apu_read_ptr + APU_INTERNAL_BUF_SIZE) % APU_INTERNAL_BUF_SIZE;

	 /* Soft Pre-roll Logic: do not start after a gap until having a small cushion (128 samples) */
         if (pre_rolling) {
            if (distance > 128) {
               pre_rolling = false;
            }
	 ...
	 /* If we have samples and are not waiting for the cushion, we read from the buffer */
         if (!pre_rolling && distance > 0) {
            last_valid_sample = apu_internal_buf[apu_read_ptr];
            apu_read_ptr = (apu_read_ptr + 1) % APU_INTERNAL_BUF_SIZE;
         } else {
	    /* Underflow! If the buffer empties, we enter pre-roll mode for the next load */	 
            if (distance == 0) pre_rolling = true;
         }

	 /* IMPORTANT: We always return last_valid_sample to maintain DC continuity */
         if (16 == apu.sample_bits)
            *buf16++ = last_valid_sample;
         else
            *buf8++ = (last_valid_sample >> 8) ^ 0x80;
      }
   }
}
...
void apu_setparams(double base_freq, int sample_rate, float refresh_rate, int sample_bits)
{
   apu.sample_rate = sample_rate;
   apu.refresh_rate = refresh_rate;
   apu.sample_bits = sample_bits;
   apu.num_samples = (int)((float)sample_rate / refresh_rate);
   if (0 == base_freq)
      apu.base_freq = (refresh_rate < 55.0f) ? APU_PAL_BASEFREQ : APU_NTSC_BASEFREQ;
   else
      apu.base_freq = base_freq;
   apu.cycle_rate = (float)(apu.base_freq / (float)sample_rate);
...
apu_t *apu_create(double base_freq, int sample_rate, float refresh_rate, int sample_bits)
{
   apu_t *temp_apu;
   int channel;
