...
static uint8 mem_readbyte(uint32 address)
{
   ...
   /* Always paged memory for ROM ($8000-$FFFF) */
   else if (address >= 0x8000)
   {
      return bank_readbyte(address);
   }
   /* Mapper handlers (WRAM $6000-$7FFF and registers $2000-$4017) */
   else
   {
...
static void mem_writebyte(uint32 address, uint8 value)
{
   ...
   /* Check for SRAM write ($6000-$7FFF) */
   if (address >= 0x6000 && address < 0x8000)
   {
      sram_dirty = 1;
      sram_last_write = (uint32)nofrendo_ticks;
   }
   ...
   /* Mapper handlers (includes WRAM $6000-$7FFF and registers $2000-$4017) */
   ...
   /* write to paged memory (if the mapper did not handle it) */
...
*/
int __attribute__((section(".itcmtext"))) nes6502_execute(int timeslice_cycles)
{
...
{
   cpu.p_reg = R_FLAG | I_FLAG;              /* I-flag ON, Reserved bit ON */
   cpu.s_reg = 0xFF;                         /* Standard stack pointer start */
