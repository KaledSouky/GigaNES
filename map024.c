...
** map24.c
**
** Mapper 24 and 26 Interface (Konami VRC6a / VRC6b) 
...
static void map24_init(void)
{
   ...
   mapper_num = mmc_getinfo()->mapper_number;
   vrcvi_set_ab_swap((26 == mapper_num) ? true : false);
}

//EUREKA! The cloud problem in the intro is finally solved. 
static void map24_hblank(int vblank)
{
   ...
   if (irq.enabled)
   {
	 ...
	 /* ** THE CHANGE IS HERE:
         ** Instead of a complete `false`, we use Konami's logic:
         ** If `irq.wait_state` is 0, the IRQ is disabled after triggering.
         ** If it is 1, it remains enabled for the next line.
         */
         irq.enabled = irq.wait_state; 

	 /*
         ** PROTECTION FOR THE GIGA R1:
         ** If the latch value is too high (close to 255), the counter will reach 256
         ** almost instantly on the next call, causing a hang.
         */
         if (irq.latch > 254) {
             irq.enabled = false;
         }
...
static void map24_write(uint32 address, uint8 value)
{
   ...
   case 0xF002:
   // This register is typically used to acknowledge the IRQ
   // and re-enable it based on the previous wait state.
      irq.enabled = irq.wait_state;
      nes_clear_irq(); // <-- clears the pending interrupt
      break;
