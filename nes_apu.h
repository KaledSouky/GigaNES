...
/* Actual hardware base frequencies */
#define APU_NTSC_BASEFREQ 1789772.7272727272
#define APU_PAL_BASEFREQ  1662607.0

#define APU_BASEFREQ APU_NTSC_BASEFREQ
...
typedef struct dmc_s
{
   ...
   /* New fields for accurate emulation */
   uint8 shift_reg;
   uint8 sample_buffer;
   int bits_remain;
   bool buffer_empty;
...
extern "C"
{
   ...
   extern void apu_setparams(double base_freq, int sample_rate, float refresh_rate, int sample_bits);
   extern apu_t *apu_create(double base_freq, int sample_rate, float refresh_rate, int sample_bits);
