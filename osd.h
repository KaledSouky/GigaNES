...
/* init / shutdown */
extern int osd_init(float hz);
extern void osd_shutdown(void);
extern int osd_main(int argc, char *argv[]);

extern int osd_installtimer(float frequency, void *func, int funcsize,
                            void *counter, int countersize);

/* input */
extern void osd_getinput(void);
extern void osd_blit_line_block(uint8_t *buf, int start_line, int num_lines);
extern void osd_getmouse(int *x, int *y, int *button);

/* NES controller specific */
extern void osd_poll_inputs(void);
extern void osd_nes_latch(uint8 value);
extern uint8 osd_nes_clock_read(uint32 address);
extern void osd_setup_video(float refresh_rate);
