...
#define NES_FIQ_PERIOD (NES_MASTER_CLOCK / NES_CLOCK_DIVIDER / NES_REFRESH_RATE)

#define NES_RAMSIZE 0x800

#define NES_SKIP_LIMIT (NES_REFRESH_RATE / 5) /* 12 or 10, depending on PAL/NTSC */

float nes_refresh_rate = 59.94f; 
int nes_scanlines_per_frame = 262;

/* Dynamic value of active cycles per scanline (60 by default, 70 for Mapper 119) */
static int g_ppu_active_cycles = 60;

#define GIGA_NTSC_FPS 59.94f
#define GIGA_PAL_FPS  50.00f
...
static float nes_audio_accum = 0.0f;
static float nes_samples_per_line = 0.0f;

void nes_update_audio_timing(void)
{
   sndinfo_t osd_sound;
   osd_getsoundinfo(&osd_sound);
   if (nes_refresh_rate > 0 && nes_scanlines_per_frame > 0)
      nes_samples_per_line = (float)osd_sound.sample_rate / (nes_refresh_rate * (float)nes_scanlines_per_frame);
}

static void nes_decode_scanline(int line, bool draw_flag)
{
   int elapsed_cycles;
   mapintf_t *mapintf = nes.mmc->intf;
   int in_vblank = (line >= 241) ? 1 : 0;
   
   /* Exact timing: NTSC 113.666 | PAL 106.5625 */
   float cycles_per_line = (nes_scanlines_per_frame == 312) ? 106.5625f : 113.6666f;
   nes.scanline_cycles += cycles_per_line;
   int cycles_to_execute = (int)nes.scanline_cycles;

   /* 1. Capture base cycles for time references (Sprite 0 hit) */
   uint32 line_base_cycles = nes6502_getcycles(false);

   /* 1a. Predict Sprite 0 hit for this line before executing CPU */
   ppu_spr0_hit_predict(line, line_base_cycles);

   /* 2. Active Phase (approx. g_ppu_active_cycles cycles): Allow mid-scanline scroll changes */
   /* We execute the drawing phase first to capture register changes */
   if (cycles_to_execute > g_ppu_active_cycles) {
      elapsed_cycles = nes6502_execute(g_ppu_active_cycles);
      nes_checkfiq(elapsed_cycles);
      cycles_to_execute -= elapsed_cycles;
      nes.scanline_cycles -= (float)elapsed_cycles;
   }

   /* 3. Mapper H-Blank (Notification for IRQs/Scrolling) */
   /* Runs just before rendering for MMC3/MMC1 synchronization */
   if (mapintf->hblank)
      mapintf->hblank(in_vblank);

   /* 4. Render scanline with the current CPU state */
   ppu_scanline(vid_getbuffer(), line, draw_flag, line_base_cycles);

   /* 5. VBlank logic / NMI (only on line 241) */
   if (241 == line)
   {
   ...
   /* 6. H-Blank Phase (Remaining cycles of the line) */
   if (cycles_to_execute > 0) {
      elapsed_cycles = nes6502_execute(cycles_to_execute);
      nes.scanline_cycles -= (float)elapsed_cycles;
      nes_checkfiq(elapsed_cycles);
   }

   /* 7. Synchronize Audio (Critical Step for Battletoads) */
   /* We process the samples proportional to this line so that the APU sees the correct banks */
   nes_audio_accum += nes_samples_per_line;
   int samples_to_do = (int)nes_audio_accum;
   if (samples_to_do > 0) {
      apu_process(NULL, samples_to_do);
      nes_audio_accum -= (float)samples_to_do;
   }

   ppu_endscanline(line);
}

static void nes_renderframe(bool draw_flag)
{
   while (nes_scanlines_per_frame != nes.scanline)
   {
      nes_decode_scanline(nes.scanline, draw_flag);
      nes.scanline++;
   }
   nes.scanline = 0;
}
...
int nes_insertcart(const char *filename, nes_t *machine)
{
   ...
   /* Set refresh rate based on ROM region */
   if (machine->rominfo->flags & ROM_FLAG_PAL) {
      nes_refresh_rate = GIGA_PAL_FPS;
      nes_scanlines_per_frame = 312;
   } else {
      nes_refresh_rate = GIGA_NTSC_FPS;
      nes_scanlines_per_frame = 262;
   }

   /*
    * The hardware was already configured in main_loop() before loading the ROM.
    * Do not call osd_setup_video again to avoid unstable reboots.
    */
   // osd_setup_video(nes_refresh_rate);

   /* Update APU with new refresh rate */
   if (machine->apu)
   {
      sndinfo_t osd_sound;
      osd_getsoundinfo(&osd_sound);
      apu_setcontext(machine->apu);
      apu_setparams(0, osd_sound.sample_rate, nes_refresh_rate, osd_sound.bps);
      apu_getcontext(machine->apu);
   }
   ...
   /*
    * Dynamic adjustment of PPU active cycles per scanline.
    * Mapper 119 (TQROM) requires 70 to avoid artifacts.
    * The rest works perfectly with 60.
    */
   g_ppu_active_cycles = (machine->rominfo->mapper_number == 119) ? 70 : 60;
   machine->ppu_active_cycles = g_ppu_active_cycles;
