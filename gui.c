...
static float gui_refresh = 60.0f; /* default to 60Hz */
...
static void gui_updatefps(void)
{
   ...
   {
      sprintf(fpsbuf, "%4d FPS /%4d%%", gui_fps, (int)((gui_fps * 100.0f) / gui_refresh));
      gui_fps = 0;
      gui_fpsupdate = false;
   }
...
void gui_sendmsg(int color, char *format, ...)
{
   ...
   msg.ttl = (int)(gui_refresh * 2.0f); /* 2 second delay */
   msg.color = color;
}

void gui_setrefresh(float frequency)
{
