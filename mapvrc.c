...
** map_vrc.c
**
** Mapper 21, 22, 23 and 25 Interface (Konami VRC4a, VRC2a, VRC2b and VRC4b)
...
static void map22_init(void)
{
   vrc_init();
   /* VRC2a has $C000 and $E000 fixed to the last two 8K banks */
   int last_bank = (mmc_getinfo()->rom_banks * 2);
   mmc_bankrom(8, 0x8000, 0);
   mmc_bankrom(8, 0xA000, 1);
   mmc_bankrom(8, 0xC000, last_bank - 2);
   mmc_bankrom(8, 0xE000, last_bank - 1);
   
   nofrendo_log_printf("Mapper 22: TwinBee 3 VRC2a Fix Applied\n");
}
...
{
   /* Swap A0 and A1 bits to translate VRC2a -> VRC2b layout */
   uint32 a0 = (address >> 0) & 0x01;
   uint32 a1 = (address >> 1) & 0x01;
   address = (address & ~0x03) | (a0 << 1) | (a1 << 0);

   /* VRC2a uses bits 1-4 for nybbles in CHR registers ($B000-$EFFF) */
   if (address >= 0xB000 && address <= 0xEFFF)
      value >>= 1;

   map23_write(address, value);
}
